...
if (missing.length > 0) {
  throw new Error(`⚠️ Critical startup failure: Missing required environment variables: ${missing.join(", ")}`);
}

if (!process.env.TWILIO_AUTH_TOKEN && (!process.env.TWILIO_API_KEY || !process.env.TWILIO_API_SECRET)) {
  throw new Error("⚠️ Critical startup failure: Either TWILIO_AUTH_TOKEN or both (TWILIO_API_KEY and TWILIO_API_SECRET) must be provided.");
}
